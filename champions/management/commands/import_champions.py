...
class Command(BaseCommand):
    help = 'Deletes existing champions and populates with fixed list'

    def handle(self, *args, **kwargs):
        Champion.objects.all().delete()
        self.stdout.write("✅ All existing champions deleted.")
        ...
        for idx, name in enumerate(champions):
            Champion.objects.create(id=idx, name=name)

        self.stdout.write(f"✅ Successfully created {len(champions)} champions.")
